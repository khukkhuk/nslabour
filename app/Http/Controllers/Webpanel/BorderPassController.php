...
class BorderPassController extends Controller
{
    protected $prefix = 'back-end';
    protected $segment = 'webpanel';
    protected $controller = 'borderpass';
    protected $folder = 'borderpass';
    ...
    public function datatable(Request $request)
    {
        ...
        ->where('title_data.type','=','borderpass')
    ...
    {   
        ...
        return view("$this->prefix.pages.$this->folder.index",[
            'js' => [
                ["type"=>"text/javascript","src"=>"backend/build/backend/borderpass.js"],
            ...
            'page' => 'index',
            'rows' => employer_data::orderby('id','desc')->get(),
        ]);
    }
    ...
    public function borderpass_status(Request $request){
        return view("$this->prefix.pages.$this->folder.index",[
            'js' => [
                ["type"=>"text/javascript","src"=>"backend/build/backend/borderpass.js"],
                ["src"=>'backend/js/sweetalert2.all.min.js'],
    ...
    public function tb_borderpass_status(Request $request){
        $like = $request->Like;
        ...
        ->leftjoin('users','title_data.created_by',"=","users.id")
        ->where('title_data.type','=','borderpass')
        ->select('*','title_data.created as title_created','users.name as created_name','employee_data.name as em_name','employee_data.surname as em_surname','employer_data.tel_number as tel','title_data.id as borderpass_id')
        ->get();
    ...
    public function tb_check_doc(Request $request){
        ...
        ->where("title_data.received_by","=",null)
        ->where('title_data.type','=','borderpass')
        ->select('*','title_data.created as title_created','title_data.updated as borderpass_updated','users.name as created_name','employee_data.name as em_name','employee_data.surname as em_surname','employer_data.tel_number as tel','title_data.id as borderpass_id')
        ->get();
        return Datatables::of($sTable)
        ->addIndexColumn()
        ->addColumn('created_at', function($row)
    ...
    public function borderpass_detail(request $request,$id){
        $result = title_data::find($id);
        // dd($result);
        return view("$this->prefix.pages.$this->folder.index",[
            'js' => [
            ...
            'folder' => $this->folder,
            'segment' => $this->segment,
            'page' => 'borderpass_detail',
            'result' => $result,
            'rows' => detail_data::where("title_id",'=',$id)->get()
        ]);
    }
    public function up_borderpass_detail(request $request){
        \DB::beginTransaction();
        try
        {  
            // dd($request); 
            ...
            {
                $lg = $file->getRealPath();
                $newLG = 'uploads/documents/borderpass/'.$request->borderpass_id.'/'.$request->detail_id.'/'.$filename;
                ...
                $data->title_id= $request->borderpass_id;
            ...
            if($data->save()){
                \DB::commit();
                return view("$this->prefix.alert.success",['url'=> url("$this->segment/$this->folder/borderpass_detail/".$request->borderpass_id)]);
            }else{
                return view("$this->prefix.alert.error",['url'=> url("$this->segment/$this->folder/borderpass_detail/".$request->borderpass_id)]);
            }
        }
        catch(\Exception $e)
        {
    ...
    public function showdoc(request $request){
        $borderpass_detail = $request->id;
        // dd($borderpass_id);
        $result = doc_data::where("detail_id",'=',$borderpass_detail)->get();
        $res = detail_data::find($borderpass_detail);
