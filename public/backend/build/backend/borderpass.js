var fullUrl = window.location.origin + '/webpanel/borderpass';
...
$(function () {
    oTable = $('#data-table').DataTable({
        ...
        iDisplayLength: 25,
        ajax: {
            url: "webpanel/borderpass/datatable",
            data: function (d) {
                d.Like={};
    ...
    oTable = $('#data-tablecheckdoc').DataTable({
        ...
        iDisplayLength: 25,
        ajax: {
            url: "webpanel/borderpass/tb_check_doc",
            data: function (d) {
                d.Like={};
                $('.myLike').each(function() {
                if( $.trim($(this).val()) && $.trim($(this).val()) != '0' ){
        ...
        columns: [
            ...
            {data: 'name',    title :'คนงาน',    className: 'text-center w30'}, //3
        ...
        rowCallback: function (nRow, aData, dataIndex) {
            ...
            $('td:eq(5)', nRow).html(''
            + aData['borderpass_updated'].substr(0,10), 
            ).addClass('input');

            $('td:eq(6)', nRow).html(''
             +'<button class="btn btn-primary" type="button" data-toggle="collapse" data-target="#collapseExample'+dataIndex+'" aria-expanded="false" aria-controls="collapseExample">ดำเนินการ</button>'
            +'<div class="collapse" id="collapseExample'+dataIndex+'"><a style="width:70%;margin:5px 0px" href="'+segment+'/'+folder+'/detail/'+aData["borderpass_id"]+'" disabled class="btn btn-sm btn-success">ดูเอกสาร</a></div>'
            +'<div class="collapse" id="collapseExample'+dataIndex+'"><a style="width:70%;margin:5px 0px" onclick="formnote('+aData['borderpass_id']+')" class="btn btn-sm btn-success">หมายเหตุ</a></div>'
            +'<div class="collapse" id="collapseExample'+dataIndex+'"><a style="width:70%;margin:5px 0px" onclick="confirm_check('+aData['borderpass_id']+')" class="btn btn-sm btn-success">ยืนยันการตรวจสอบ</a></div>'
           ).addClass('input');
    ...
    oTable = $('#data-tableborderpass').DataTable({
        "sDom": "<'row'<'col-sm-12'tr>><'row'<'col-sm-5'i><'col-sm-'p>>",
        processing: true,
        serverSide: true,
        stateSave: true,
        ...
        iDisplayLength: 25,
        ajax: {
            url: "webpanel/borderpass/tb_borderpass_status",
            data: function (d) {
                d.Like={};
                $('.myLike').each(function() {
                if( $.trim($(this).val()) && $.trim($(this).val()) != '0' ){
        ...
        columns: [
            ...
            {data: 'received_by',    title :'คนส่งงาน',    className: 'text-center w30'}, //4
            {data: 'received',    title :'วันที่ส่งงาน',    className: 'text-center w30'}, // 1
        ...
        rowCallback: function (nRow, aData, dataIndex) {
            ...
            if(aData['select_date']==null){
                received_by="-"
            }else{
                received_by=aData['select_date']
            }
            $('td:eq(4)', nRow).html(''
            + received_by, 
            ).addClass('input');

            if(aData['received']==null){
                received = "-"
            }else{
                received = aData['received'].substr(0, 10)
            }
            $('td:eq(5)', nRow).html(''
            + received, 
            ).addClass('input');
            ...
            $('td:eq(6)', nRow).html(''
            +'<a style="width:70%;margin:5px 0px" href="'+segment+'/'+folder+'/borderpass_detail/'+aData["borderpass_id"]+'" class="btn btn-sm btn-success">ดูรายละเอียดการดำเนินการ</a>'
            ).addClass('input');
        }
    });
    $('.myWhere,.myLike,.myCustom,#onlyTrashed').on('change', function(e){
