var fullUrl = window.location.origin + '/webpanel/renew';
...
$(function () {
    ...
    oTable = $('#data-table-report').DataTable({
        ...
        columns: [
            ...
            {data: 'created',    title :'วันที่รายงานตัวครั้งต่อไป',    className: 'text-center w40'}, // 1
            {data: 'name_th',    title :'ชื่อนายจ้าง',    className: 'text-center w30'}, //2
        ...
        rowCallback: function (nRow, aData, dataIndex) {
            ...
            $('td:eq(8)', nRow).html(''
            +'<a style="width:70%;margin:5px 0px" href="'+segment+'/'+folder+'/edit_report/'+aData["report_id"]+'"class="btn btn-sm btn-success">แก้ไขข้อมูล</a>'
            ).addClass('input');
        }
    });
...
$(document).on('change','input', '#employer_name', function(){
    ...
    $.ajax({
        url:'webpanel/renew/get_employer',
        method:"post",
        data:{
            'name_th':$(this).val(),
        },
        success:function(result){
            ...
            $.ajax({
                url:'webpanel/renew/get_employee',
                method:"post",
                data:{
                    'id':result[0].id,
                },
...
$(document).on('change','input', '#employee_name', function(){
    $.ajax({
        url:'webpanel/renew/employee_data',
        method:"post",
        data:{
            'name_th':$(this).val(),
